...
CREATE TABLE `color` (
  ...
  `model_id` int(11) NOT NULL COMMENT '商品型号id(外键)',
...
CREATE TABLE `order` (
  ...
  `order_type` int(11) NOT NULL DEFAULT '0' COMMENT '订单状态(0未付款,1付款成功,-1取消订单)',
...
CREATE TABLE `product_info` (
  ...
  `brand_id` int(11) NOT NULL COMMENT '商品品牌编号(外键)',
  `model_id` varchar(255) NOT NULL COMMENT '型号编号(外键)',
  ...
  `product_collect_num` int(11) NOT NULL COMMENT '商品收藏量',
...
CREATE TABLE `stock` (
  ...
  `brand_id` int(11) NOT NULL COMMENT '商品品牌id(外键)',
  `store_id` int(11) NOT NULL COMMENT '存储规格id(外键)',
  `model_id` int(11) NOT NULL COMMENT '型号id(外键)',
