...
<title>商品列表 - 商品管理 - 找靓机后台管理系统 v3.0</title>
</head>
<body>
<!--_header 作为公共模版分离出去-->
...
<section class="Hui-article-box">
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加商品','${pageContext.request.contextPath }/Commodity/add.do','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加商品</a></span> <span class="r">共有数据：<strong>${page.totalRecord}</strong> 条</span> </div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					...
					<tbody>
					<c:forEach var="test" items="${page.list}">
						<tr class="text-c">
							<td><input type="checkbox" value="1" name=""></td>

							<td>${test.productId}</td>
							<td><u style="cursor:pointer" class="text-primary" onclick="member_show('${test.productName}','member-show.html','10001','360','400')">${test.productName}</u></td>
							<td><u style="cursor:pointer" class="text-primary" onclick="member_show('${test.brand.brandId}','member-show.html','10001','360','400')">${test.brand.brandName}</u></td>
							<td><img src="${test.productImage}" style="width: 90px; height: 90px;" /> </td>
							<td>${test.productInfo}</td>
							<td>${test.createTime}</td>
							<td>${test.stock.stockNumber}</td>
							<c:choose   >
							<c:when test="${test.productStatus==1}">
								<td class="td-status"><span class="label label-success radius">已上架</span></td>
							</c:when>
								<c:when test="${test.productStatus==0}">
									<td class="td-status"><span class="label label-defaunt radius">已上架</span></td>
								</c:when>
							</c:choose>
							<td>${test.productSaleNum}</td>
